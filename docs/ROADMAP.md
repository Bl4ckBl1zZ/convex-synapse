...
- [x] **Phase 3 polish on the in-iframe deployment picker.** Same
  ...
  + cross-origin walls (the two ideas that hit them).
  Tests: +5 Playwright (46).
...
- [x] **In-header deployment picker on the Convex Dashboard (Strategy E).**
  ...
  for a future in-iframe Strategy B if we ever take that path.
...
- [x] **Auto-update from the dashboard ✅ DONE (v1.1.0)** — yellow banner in the dashboard layout polls `GET /v1/admin/version_check` (cached fetch of GitHub `/releases/latest`, 15min TTL) hourly and surfaces "vX.Y.Z available" when the running version is older. Click → modal with rendered release notes + "Upgrade now" → POST `/v1/admin/upgrade` → dashboard streams `setup.sh --upgrade` logs from the daemon's `/status` endpoint until the synapse-api container restarts; on connection drop the modal flips to "page will reload in 90s" and force-reloads back into the new build. Architecture: small Python 3 HTTP daemon at `/run/synapse/updater.sock` (unix socket — no TCP exposure), installed by `setup.sh`'s new `phase_install_updater` as a systemd unit (`User=root`, lives outside docker compose so it survives the rebuild it orchestrates). Self-update guard `SYNAPSE_UPDATER_NO_RESTART=1` keeps phase_install_updater from killing the parent daemon mid-upgrade. Audit log: `upgradeStarted` per click. Tests: 12 Go integration cases (cache hit, GitHub down fallback, semver edge cases, updater pass-through, audit log, admin gate); 9 bats cases for `phase_install_updater`. (PR #46)
- [x] **Deploy keys ✅ DONE (v1.0.3)** — per-deployment named admin keys mirroring Convex Cloud's "Personal Deployment Settings → Deploy Keys" UX. Each CI integration gets its own credential ("vercel", "github-actions", "claudin") with created_at, created_by, audit log entries on create + revoke. Backend mints a fresh admin key per deploy_key via the Convex backend's `generate_key` binary; we store sha256 + 8-char prefix only (GitHub-PAT model — full value shown once at creation). Dashboard panel sits next to "Show CLI credentials" with a `+ Create` button + table view + revoke. Revoke rotates the deployment's INSTANCE_SECRET, updates the primary admin key, recreates the managed container, and marks all active deploy keys for that deployment revoked; this is deployment-wide because Convex admin keys are statelessly verified against INSTANCE_SECRET. Migration 000009 repurposes the orphaned v0 `deploy_keys` table. Go integration tests cover created/listed/revoked + duplicate-name + name-reusable + cross-deployment + audit + credential rotation. (PR #45)
