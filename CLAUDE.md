...
| `docs/` | ARCHITECTURE, ROADMAP, QUICKSTART, API, DESIGN, HA_TESTING, PRODUCTION, HANDOFF |
...
| **Per-deployment backups (v1.26)**: zip real de `npx convex export` rodado num container node descartável (mesma maquinaria do upgrade_to_ha) → volume fixo `synapse-backups` (compose monta em /backups = SYNAPSE_BACKUP_DIR); restore = `convex import --replace` (destrutivo, admin + confirm). Jobs `backup`/`restore` na provisioning_jobs (migration 000035); sweeper advisory-locked (LockBackupSweeper) faz daily schedule + poda por retenção (1-90). Endpoints sob `/v1/deployments/{name}`: backups (GET list/POST create), download (stream), restore, delete, backup_settings. **Gotchas aprendidos**: convex CLI exige cwd com package.json contendo convex nas deps (stub `{"dependencies":{"convex":"*"}}` — MigrateSnapshot tinha o mesmo bug latente, corrigido); logs do Docker são multiplexados (stdcopy) e os headers binários com NUL quebram TEXT do Postgres — demux antes de gravar | `internal/docker/backup.go`, `internal/provisioner/backup.go`, `internal/api/backups.go`, `internal/backups/sweeper.go`, `dashboard/components/BackupsPanel.tsx` |
| **Resource limits (v1.26)**: `cpus`/`memoryMb` opcionais no create_deployment → `DeploymentSpec.CPUs/MemoryMB` → `HostConfig.Resources` (NanoCPUs/Memory); NULL = ilimitado (migration 000034). Recreates (domain rebake) recarregam os limites do row — rebake nunca destampa um container. Resize: `POST /v1/deployments/{name}/update_resources` (full desired state, recria o container, volume preservado); adopted/HA/remote → 409 com códigos estáveis. Badge + dialog de Resize na row do projeto | `internal/api/deployments.go::updateResources/validateResourceLimits`, `internal/docker/provisioner.go` (HostConfig), `internal/provisioner/worker.go::buildSpec`, migration 000034 |
| **Adopt existing (v1.27 hardening)**: registra backend Convex externo (probe /version + check_admin_key, sem seguir redirects — SSRF posture documentada no probeAdoptedBackend; nome user-supplied validado como DNS label). Health worker monitora adopted via HTTP `GET <url>/version` (2 falhas consecutivas → stopped + alerta deployment-down exactly-once; recovery silencioso) — antes o status ficava congelado em "running" pra sempre. `POST /v1/deployments/{name}/update_adopted` re-aponta URL/admin key **in place** (re-probe antes de gravar; promove stopped→running; audit `updateAdoptedDeployment`) — necessário porque delete é soft e o nome é UNIQUE total: re-adopt nunca reusa o nome. Recusas todas 409 (`cannot_{restart,resize,backup,upgrade,reissue}_adopted`, `deploy_keys_unsupported_for_adopted` incl. list, `domains_unsupported_for_adopted`; tls_ask filtra adopted). `backend_version` para adopted sonda a URL externa. UI: Restart/Resize/DeployKeys/Domains/Backups escondidos no card adopted + nota explicativa + botão "Edit URL / key"; modal adopt oferece os 4 tipos | `internal/api/deployments.go::adoptDeployment/updateAdopted/probeAdoptedBackend`, `internal/health/worker.go::sweepAdopted`, `internal/api/{domains,tls_ask,backend_version}.go` |
...
| **Deployment-down alerts (v1.26)**: health worker fires `Alerter.DeploymentDown` on the deployment-level transition to stopped/failed (UPDATE…RETURNING prev.status — exactly once per down event; auto-restart-recovered blips never alert). `alert.Notifier` fans out detached + best-effort: email to the owning team's ADMINS (via shared `email.ResolveSender` — DB email_settings wins over .env) + generic webhook (payload carries both `text`/Slack and `content`/Discord + structured fields). Config: **Admin → Alerts** → `/v1/admin/alert_settings` (migration 000032 singleton; row wins ENTIRELY over `SYNAPSE_ALERT_WEBHOOK_URL`, even with empty webhook; URL never echoed — masked hint only; POST `webhookUrl` absent=keep, empty=clear) | `internal/alert/`, `internal/health/worker.go::recomputeDeploymentStatus`, `internal/api/alert_settings.go`, `internal/email/resolve.go`, `dashboard/components/AlertSettingsPanel.tsx` |
...
| **Site origin / HTTP actions (site-origin)**: a Convex backend opens TWO ports — cloud (3210, client API + HTTP actions under `/http/`) and the site proxy (3211, HTTP actions at natural paths). Synapse used to expose only 3210 and set `CONVEX_SITE_ORIGIN == CONVEX_CLOUD_ORIGIN`, so Better Auth `/api/auth/*`, webhooks, and `/engine/*` 404'd externally. Now each deployment gets a site host: `<name>.site.<BASE_DOMAIN>` (→3211) via a sibling Caddy wildcard, plus a per-deployment `role='site'` custom domain. `deploymenturl.Computer.Site()` computes it; provisioner bakes `CONVEX_SITE_ORIGIN`; `proxy.ResolveAllSite` re-ports 3210→3211 (cloud path untouched, host-port mode → 501); `tls_ask` approves `<name>.site.<base>`; CLI writes the distinct `NEXT_PUBLIC_CONVEX_SITE_URL`; dashboard shows the real HTTP Actions URL. Migration 000022 adds `role='site'`. **Institutional memory**: `docs/CONVEX_SITE_ORIGIN.md`. | `synapse/internal/deploymenturl/url.go`, `synapse/internal/docker/provisioner.go`, `synapse/internal/proxy/proxy.go`, `synapse/internal/api/{tls_ask,domains,deployments}.go`, `synapse/internal/db/migrations/000022_deployment_domains_site_role`, `cli/lib/env-file.js`, `installer/templates/caddy.wildcard` |
| **Function-runtime env vars (v1.17.0)**: the "Default environment variables" project-settings panel now writes to the **Convex function runtime env store** — the same one `npx convex env set` and the Convex Dashboard env panel use, read by functions via `process.env.NAME` — instead of container ENV (which functions never read). Pushes via Convex backend's `POST /api/update_environment_variables` API authenticated with the deployment's admin key; **no container restart**. Auto-syncs on every save; manual **Re-sync to deployments** button (renamed from "Apply to existing") retries any failed deployments. CLI-managed names (`CONVEX_SELF_HOSTED_URL`, `NEXT_PUBLIC_CONVEX_URL`, `CONVEX_DEPLOY_KEY`, etc) are silently filtered so a paste-mistake doesn't override the framework's values. Migration is lazy: existing `project_env_vars` rows push to the function store on the next operator save or Re-sync click. New audit action `updateFunctionEnvVars`. Three env-var categories now documented in `docs/CONVEX_SITE_ORIGIN.md#environment-variable-categories-v117`. | `synapse/internal/convexenv/`, `synapse/internal/api/projects.go::updateEnvVars`, `synapse/internal/api/projects.go::syncEnvToDeployments`, `synapse/internal/provisioner/worker.go::runJob`, `dashboard/components/EnvVarsPanel.tsx` |
| **Remote Hosts (v1.18.0)**: multi-host deployment via Headscale (self-hosted Tailscale control plane) + `install-agent.sh` one-liner + per-host ed25519 SSH keys encrypted via `crypto.SecretBox` + `RemoteClient` SSH dispatch with `synapse-deployer-exec` subcommand whitelist. New endpoints: `POST /v1/hosts/{id}/remote_setup`, `GET /v1/install_agent/config`. Schema: migrations `000024`–`000026` (`hosts.tailnet_addr`/`ssh_pubkey`/`ssh_user`/`ssh_port`/`is_remote`, `hosts.ssh_privkey_encrypted`/`ssh_privkey_fingerprint`, `deployments.host_id NOT NULL`). New packages: `synapse/internal/headscale`, `internal/hostssh`, `internal/sshprov`, `internal/docker/remote.go`. `installer-agent/` scaffolding for the per-VPS install + hardened systemd unit + sshd `Match User` drop-in + forced-command wrapper. Caddy `headscale.<base-domain>` subdomain block + `setup.sh --enable-headscale` flag (opt-in; rejected under `--no-tls`). Full docs: `docs/REMOTE_HOSTS.md` + `docs/SECURITY_REMOTE_HOSTS.md`. | `synapse/internal/{headscale,hostssh,sshprov,docker/remote.go,api/remote_setup.go,api/install_agent.go,provisioner/worker.go}`, `installer-agent/`, `install-agent.sh`, `installer/install/headscale.sh`, `installer/templates/{caddy.headscale.fragment.tmpl,headscale.config.yaml.tmpl}` |
| Dashboard (20 e2e tests, real pages, HA toggle + badge) | `dashboard/` |
| **Dashboard PT-BR localization (v1.21.0)**: the entire dashboard interface is translatable EN ⇄ pt-BR. Lightweight in-house i18n (no `next-intl`/`i18next` dep) using the **"English-as-key"** model — every user-facing string is wrapped in `t("English source")`; a 1157-key pt-BR dictionary maps English→Portuguese; any missing key falls back to the English source (UI never blank). Default locale is **English**, so the rendered DOM is byte-identical to pre-i18n (the 568 text-based Playwright assertions stay green — they run in en-US). Locale resolved **server-side** in the root layout (`synapse_locale` cookie → `Accept-Language` → `en`) so the first paint is correct with no flash; a Brazilian browser auto-lands in pt-BR. `EN \| PT` switcher (`LocaleSwitcher`) lives in the TopBar (authed pages) + a floating pill on auth/setup pages. Dev jargon (deployment, deploy, token, host, proxy, CLI, …) and product names (Synapse, Convex, Cells, Headscale) are kept in English inside translated sentences. The 61 client components were wrapped; server components (root layout, docs — which already had `[locale]`) handled separately. | `dashboard/lib/i18n/{locale.ts,index.tsx,server.ts,dictionary.pt-BR.ts}`, `dashboard/components/LocaleSwitcher.tsx`, `dashboard/app/layout.tsx`, every `dashboard/{app,components}/**/*.tsx` with UI text |
